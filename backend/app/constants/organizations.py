...
ROLE_MANAGER_REQUIREMENTS = {
    ...
    # Pre-Sales / Technical hierarchy
    SOLUTION_ENGINEER: {PRE_SALES_MANAGER},
    DELIVERY: {PRE_SALES_MANAGER},

    # Manager-level roles do not require another manager.
    SALES_MANAGER: set(),
...
def get_organizations_for_roles(roles):
    ...
    return sorted(
        {
            ROLE_ORGANIZATIONS[role]
            for role in roles
            if role in ROLE_ORGANIZATIONS
        }
    )
...
def get_required_manager_roles(roles):
    """
    Return the manager roles required for the selected roles.

    If a user has multiple roles, all applicable manager
